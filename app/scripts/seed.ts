/**
 * Ayala Land Payments Portal — Comprehensive Seed Script
 *
 * Populates all 18 tables with realistic synthetic data:
 *   - 30 tenant customers at real Ayala Land properties
 *   - 20 property manager customers
 *   - 1-2 contracts per customer
 *   - 3-6 months of invoices (Oct 2025 – Mar 2026)
 *   - Payment records for paid/partial invoices
 *   - 20 suppliers from the Ayala Land ecosystem
 *   - 2-3 POs per supplier linked to real Ayala Land projects
 *   - Goods receipts, supplier invoices, outgoing payments
 *   - 3-way matches (70% FULL, 15% PARTIAL, 15% MISMATCH)
 *   - ML scores: payer_segments, delinquency_scores, credit_risk_scores, payment_patterns
 *   - 5 insight cards, 3 sample documents, 3 escalations
 *   - Cash flow forecasts
 *
...
if (!PROXY_URL || !PROXY_KEY) {
  console.error('Missing SUPABASE_SQL_PROXY_URL or SUPABASE_SQL_PROXY_KEY env vars')
  process.exit(1)
}
