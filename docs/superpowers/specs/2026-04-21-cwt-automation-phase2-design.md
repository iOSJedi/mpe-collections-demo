...
| 10 | Ask | Add the Withholding Tax module to the Collections engagement. | Phase-2 scope box: re-uses existing schema + documents pipeline + Gemini stack. Three deliverables: enrollment portal, auto-issuance engine, AR QAP export. Next step: SOW amendment. | Three deliverable chips (no timing), Phase-1 "done" vs Phase-2 "proposed" badges only |
