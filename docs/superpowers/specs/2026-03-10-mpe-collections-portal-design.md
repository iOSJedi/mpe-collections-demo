...
4. **Collections Management**
   - Page: Collections Worklist — Prioritized customer list by overdue amount
...
5. **Intelligence**
   ...
   - Page: Analytics — Cash flow forecasts, collection performance trends, credit risk heatmaps
...
- Payment amount: toggle between Pay Full and Pay Partial (editable amount)
- Payment methods:
  - **Credit / Debit Card** — Stripe PaymentIntent flow, Stripe Elements for card input, NO Stripe branding visible
  - **BPI Bank Transfer** — Emulated; shows BPI account details, customer transfers externally, then uploads proof-of-payment via the Customer Emulator
- On successful card payment: update invoice balance_remaining, set status to PARTIAL or PAID, create incoming payment record
- On partial payment: generate new QR for remaining balance
...
- **Tabs:**
  - My Invoices — list of customer's invoices with status (OVERDUE/PENDING/PAID), "Show QR" and "Upload Proof" actions per invoice
  - QR Codes — displays generated QR code for selected invoice, scannable by phone camera
...
### Outcomes
- **All checks pass:** Auto-create incoming payment record, update invoice balance, mark document as validated
- **Discrepancy found:** Create escalation record, add to escalation queue, notify via Firebase RTDB
